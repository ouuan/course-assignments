...
(
    ...
    input logic [47:0] mac_address[3:0],
    input logic [127:0] ip_address[3:0],
    input logic [127:0] solicited_ip_address[3:0],
    input logic [47:0] solicited_mac_address[3:0],
    ...
    always @(*) begin
        ...
        for (integer i = 0; i < 4; ++i) begin
            if (touch_btn[i]) begin
                led = mac_address[i][47:40];
            end
        end
    end

    logic [127:0] write_ip;
    logic [47:0]  write_mac;
    logic [ID_WIDTH - 1:0] write_interface;
    logic we;
    logic write_exist;

    logic [127:0] read_ip;
    logic [47:0]  read_mac;
    logic [ID_WIDTH - 1:0] read_interface;
    logic read_exist;
    ...
    nd_cache #(
        ...
        .we(we),
        .write_exist(write_exist),

        .read_ip(read_ip),
        .read_mac(read_mac),
        .read_interface(read_interface),
        .read_exist(read_exist)
    );
    ...
    // README: Here, we use a width upsizer to change the width to 88 bytes
    ...
    receive_nd #(
        ...
        .interface_mac_address(mac_address),
        .solicited_ip_address(solicited_ip_address),
        .solicited_mac_address(solicited_mac_address)
    );

    frame_beat s2;
    wire s2_ready;

    forward_engine forward_engine_i (
        .clk(eth_clk),
        .rst(reset),
        .in(s1),
        .in_ready(s1_ready),
        ...
        .interface_ip_address(ip_address),
        .interface_mac_address(mac_address),
        .nd_cache_read_ip(read_ip),
        .nd_cache_read_interface(read_interface),
        .nd_cache_read_mac(read_mac),
        .nd_cache_read_exist(read_exist),
        .bram_node_addr,
        .bram_node_data,
        .bram_entry_addr,
        .bram_entry_data,
