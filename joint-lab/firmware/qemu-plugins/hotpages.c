    ...
    if (counts && g_list_next(counts))
    {
        ...
        for (i = 0; i < limit && it->next; i++, it = it->next)
        {
            ...
            g_string_append_printf(
                report, "0x%016" PRIx64 ", 0x%04x, %" PRId64 ", 0x%04x, %" PRId64 "\n",
                rec->page_address, rec->cpu_read, rec->reads, rec->cpu_write, rec->writes);
        }
        g_list_free(it);
    }
...
static void vcpu_haddr(unsigned int cpu_index, qemu_plugin_meminfo_t meminfo, uint64_t vaddr,
                       void *udata)
{
...
static void vcpu_tb_trans(qemu_plugin_id_t id, struct qemu_plugin_tb *tb)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        qemu_plugin_register_vcpu_mem_cb(insn, vcpu_haddr, QEMU_PLUGIN_CB_NO_REGS, rw, NULL);
    }
}
